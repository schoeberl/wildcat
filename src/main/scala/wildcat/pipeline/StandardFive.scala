 ...
 *
 */
class StandardFive() extends Wildcat() {
  ...
  val forwardReg = RegInit(0.U.asTypeOf(forward))
...
//  val pcReg = RegInit(-4.S(32.W).asUInt)
  ...
  memWbReg.regData := exMemReg.wbData
  memWbReg.memData := io.dmem.rdData
  memWbReg.isLoad := exMemReg.isLoad

  // write back mux in memory stage
  writeBackData := Mux(memWbReg.isLoad, memWbReg.memData, memWbReg.regData)
  writeBackDest := memWbReg.wbDest
  writeBackEna := memWbReg.valid

  // Forwarding register values to ALU and memory?
  forwardReg.valid := memWbReg.valid
  forwardReg.wbDest := memWbReg.wbDest
  forwardReg.wbData := writeBackData
